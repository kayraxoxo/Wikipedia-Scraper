...
with st.sidebar:
    ...
    st.session_state["gewaehlte_sprache_code"] = LANGUAGES[lang_auswahl]
    ...
    if kat_eingabe:
        ...
        if st.button("Seiten laden / Mehr laden"):
            with st.spinner("Lade Kategorie..."):
                mitglieder, token, fehler = lade_kategorie_mitglieder(
                    st.session_state["aktuelle_kategorie"], 
                    lang=st.session_state["gewaehlte_sprache_code"], 
                    fortsetzung=st.session_state.get("kat_continue_token")
                )
                if fehler:
                    st.error(fehler)
                else:
...
def fetch_search_live(query: str):
    ...
    if not query:
        return []
    # Wenn URL, direkt zurückgeben (als Tuple für streamlit_searchbox)
    if query.startswith("http://") or query.startswith("https://"):
        return [(query, query)]
        
    lang = st.session_state.get("gewaehlte_sprache_code", "de")
    ergebnisse = fetch_search_suggestions(query, lang=lang)
    # Gibt eine Liste von Tuples zurück: (Anzeige-Name, URL-Wert)
    return [(e["titel"], e["url"]) for e in ergebnisse]
...
fehler = st.session_state.get("fehler")
        ...
        with tab_mindmap:
            ...
            if not daten['struktur']:
                ...
                try:
                    ...
                    svg_str = _re.sub(r'(<svg)', r'\1 style="width:auto;height:auto;display:block;margin:0 auto;"', svg_str, count=1)
                ...
                if daten['kategorien']:
                    for kat_idx, kat in enumerate(daten['kategorien']):
                        if st.button(kat, key=f"kategorie_browser_{kat_idx}"):
                            st.session_state["aktuelle_kategorie"] = kat
                            st.session_state.pop("kat_continue_token", None)
                            st.session_state.pop("kat_seiten_liste", None)
                            st.info("Kategorie in die Seitenleiste geladen. Bitte dort auf 'Seiten laden' klicken.")
                else:
